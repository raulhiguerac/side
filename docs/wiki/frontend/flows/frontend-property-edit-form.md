...
Vista de edición en `/properties/:id/edit` (`EditPropertyView.vue`), accesible desde `MyPropertiesView`. A diferencia del form de creación (wizard de 4 pasos), es una sola pantalla en 2 columnas: fotos + info fija a la izquierda, estadísticas + form editable a la derecha. La view solo orquesta (fetch + `form` ref + save); todo el markup vive en 5 componentes hijos bajo `components/properties/edit/`. Reutiliza la convención `:form`/`@update:form` del create flow (ver [[frontend-property-create-form]]) y el componente `PropertyPhotoGrid` que ya usaba `PropertyDetailView`.
...
| `PropertyPhotosCard.vue` | Envuelve `PropertyPhotoGrid.vue` (el mismo componente compartido que usa `PropertyDetailView`) | `property: PropertyDetail \| null` |
...
`EditPropertyView.vue` queda en ~96 líneas: `route`/`router`, `isLoading`/`property`/`locationLabel`/`form` refs, un `onMounted` que hace fetch + popula `form`, y un `handleSave` (hoy mock — falta cablear `PATCH /v1/properties/{id}`).
...
- `handleSave()` es mock (`console.log`) — falta cablear `PATCH /v1/properties/{id}` con el `form` actual.
- Fotos: solo lectura (reusa `PropertyPhotoGrid`). Falta upload de nuevas / borrado de existentes — requiere `DELETE /v1/properties/{id}/images`, sin cablear.
