...
4. Construye `updates = [{"id": n.id, "geom": geom_from_geojson(feature['geometry'])} for n in neighborhoods]`. `geom_from_geojson` vive en [`shared/helpers/geometry.py`](backend/catalog-service/src/app/services/shared/helpers/geometry.py).
