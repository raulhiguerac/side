...
Para que `/by-coordinates` sea drop-in replacement de `/resolve-neighborhood`, falta un trabajo de refactor:
- Agregar `BackgroundTasks.add_task(poi_uc.execute, ...)` al endpoint `/by-coordinates` (hoy solo lo dispara `/resolve-neighborhood`).
- Migrar consumers existentes.
- Eliminar `ResolveNeighborhoodUseCase`, `GeoreferentiationClient` (mapbox SDK wrapper en `integrations/`), `GeocodingGateway` port, `GeocodingAdapter` adapter, schema `GeocodingResult`, env `MAPBOX_API_KEY` del backend.

Este ADR documenta **el estado deseado** post-refactor. El código actual aún tiene `/resolve-neighborhood` funcionando.
...
1. Agregar `BackgroundTasks` para `ResolvePoiUseCase` al endpoint `/by-coordinates`.
2. Actualizar `properties-service` para llamar `/by-coordinates` en lugar de `/resolve-neighborhood` al crear listing.
...
- `/geo-resolution/by-coordinates` **no** dispara el `ResolvePoiUseCase` al 2026-05-21 — gap del refactor ([api/routes/geo_resolution.py:43-49](backend/catalog-service/src/app/api/routes/geo_resolution.py#L43-L49)).
