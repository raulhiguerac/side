<template>
  <BaseModal size="3xl" :model-value="isOpen" @update:model-value="$emit('close')">
    <div class="relative flex items-center">
      <button
        class="absolute left-0 z-10 w-9 h-9 rounded-full bg-white/90 hover:bg-white flex items-center justify-center"
        @click="currentSlide = currentSlide === 0 ? images.length - 1 : currentSlide - 1"
      >
        <ChevronLeft :size="18" />
      </button>
      ...
      <Carousel v-model="currentSlide" :wrap-around="true" class="w-full">
        <Slide v-for="(img, i) in images" :key="i">
          <img :src="img.url" :alt="`Foto ${i + 1}`" class="w-full max-h-[70vh] object-contain" />
        </Slide>
      </Carousel>

      <button
        class="absolute right-0 z-10 w-9 h-9 rounded-full bg-white/90 hover:bg-white flex items-center justify-center"
        @click="currentSlide = currentSlide === images.length - 1 ? 0 : currentSlide + 1"
      >
...
const { isOpen, images, startIndex } = defineProps<{
  isOpen: boolean;
