<template>
  <div>
    ...
    <p class="text-brand-muted text-sm mb-6">
      Selecciona el tipo de propiedad y las condiciones del negocio.
    </p>
    ...
    <div class="mb-4">
      <label
        ...
        :class="
          attempted && !form.property_type
            ? 'text-red-500'
            : 'text-brand-muted uppercase tracking-wide'
        "
        >Tipo de propiedad<span
          v-if="attempted && !form.property_type"
      ...
      <div
        ...
        :class="
          attempted && !form.property_type
            ? 'ring-2 ring-red-300 rounded-2xl p-1'
            : ''
        "
      >
        <button
          ...
          :class="[
            'group relative flex flex-col items-start gap-2 p-4 rounded-2xl border-2 text-left transition-all duration-200',
        ...
        >
          ...
          <div
            v-if="form.property_type === opt.value"
            class="absolute top-3 right-3"
          >
            <div
              class="w-5 h-5 rounded-full bg-brand-primary flex items-center justify-center"
            >
    ...
    <div class="mb-4">
      <label
        class="block text-xs font-semibold mb-3"
        :class="
          attempted && !form.listing_type
            ? 'text-red-500'
            : 'text-brand-muted uppercase tracking-wide'
        "
        >Tipo de negocio<span
          v-if="attempted && !form.listing_type"
          class="ml-1 normal-case"
          >— requerido</span
        ></label
      >
      ...
      >
        ...
        >
          ...
          <div
            v-if="form.listing_type === opt.value"
            class="absolute top-3 right-3"
          >
            <div
              class="w-5 h-5 rounded-full bg-brand-primary flex items-center justify-center"
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
      <div>
        <label
          class="block text-xs font-semibold mb-3"
          :class="
            attempted && !form.condition
              ? 'text-red-500'
              : 'text-brand-muted uppercase tracking-wide'
          "
          >Condición<span
            v-if="attempted && !form.condition"
            class="ml-1 normal-case"
            >— requerido</span
          ></label
        >
        <div
          ...
          :class="
            attempted && !form.condition
              ? 'ring-2 ring-red-300 rounded-xl p-1'
              : ''
          "
        >
          <button
            ...
            :class="[
              'flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border-2 text-sm font-medium transition-all duration-200',
      ...
      <div>
        ...
        <div class="grid grid-cols-6 gap-1.5">
          <button
            ...
            :class="[
              'flex items-center justify-center py-2.5 rounded-xl border-2 text-xs font-semibold transition-all duration-200',
              form.currency === c
    ...
    <div class="grid grid-cols-3 gap-3">
      <div v-for="field in numericFields" :key="field.key">
        <label
          class="block text-xs font-semibold text-brand-muted uppercase tracking-wide mb-2"
          >{{ field.label }}</label
        >
        <div class="relative">
          <component
          ...
          <input
            ...
            @input="
              emit('update:form', {
                ...form,
                [field.key]: +($event.target as HTMLInputElement).value || null,
              })
            "
            :type="'number'"
            :min="field.min"
            ...
            :class="[
              ...
              attempted && !(form as any)[field.key]
...
defineProps<{ form: CreatePropertyForm; attempted?: boolean }>();
