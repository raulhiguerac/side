<template>
  <div class="w-full px-[5%] sm:px-[8%] lg:px-[10%] py-8">
    ...
    <!-- Profile header -->
    <div class="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-5 bg-white border border-brand-divider rounded-2xl p-5">
      <img
      ...
      <div class="flex flex-col gap-1.5 flex-1">
        ...
        <div class="flex flex-wrap items-center gap-3 text-brand-text text-sm mt-1">
      ...
      <div class="flex gap-2 shrink-0">
        <button class="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-brand-primary text-white text-sm font-medium hover:bg-green-600 transition">
          <MessageCircle class="w-4 h-4" /> WhatsApp
        </button>
        <button class="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-brand-border text-brand-text text-sm font-medium hover:bg-brand-primary-light transition">
    ...
    <!-- Listings -->
    <div v-if="cards.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
      <PropertyCard
        v-for="card in cards"
        :key="card.id"
        :property="card"
    ...
    <div class="flex justify-center items-center gap-4 mt-10">
      <button
        :class="['px-5 py-2 rounded-xl border border-brand-border text-sm font-medium text-brand-text transition hover:bg-brand-primary-light', { invisible: !hasPrev }]"
        @click="prev"
      >
        ← Anterior
      </button>
      <button
        :class="['px-5 py-2 rounded-xl border border-brand-border text-sm font-medium text-brand-text transition hover:bg-brand-primary-light', { invisible: !hasNext }]"
        @click="next"
      >
        Siguiente →
      </button>
...
const { listings, hasMore, fetchUserListings, previousListings } = useProfileListings();
...
const activeListingsCount = computed(() => (hasMore.value ? "+20" : listings.value.length));
